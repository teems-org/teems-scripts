...
model_specs <- teems_model(tab_file = "GTAPv6.2",
                           var_omit = c("atall", "tfd", "avaall", "tf", "tfm", "tgd", "tgm", "tpd", "tpm"))

set_specs <- teems_sets(set_input = "~/dat/GTAP/v10A/flexagg10AY14/gsdset.har",
                        REG = "AR5",
                        TRAD_COMM = "macro_sector",
                        ENDW_COMM = "labor_agg")

data_specs <- teems_data(dat_input = "~/dat/GTAP/v10A/flexagg10AY14/gsddat.har",
                         par_input = "~/dat/GTAP/v10A/flexagg10AY14/gsdpar.har")

model_name <- "GTAP-v6.2_v10"
